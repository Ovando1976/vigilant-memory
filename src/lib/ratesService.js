import { apiFetch, BACKEND_ENABLED } from "./backend";
...
export async function resolveRoute({
  pickup, dropoff, pickupCoords, dropoffCoords, island, signal,
}) {
  ...
  if (BACKEND_ENABLED) {
    try {
      return await apiFetch("/api/resolve-route", {
        ...
        body: JSON.stringify({ pickup, dropoff, pickupCoords, dropoffCoords, island }),
        signal,
      });
    } catch (e) {
  ...
  const fromName = pickup || nearestNamed(fromCoords, islandOf(fromCoords))?.name || "Custom A";
  const toName   = dropoff || nearestNamed(toCoords, islandOf(toCoords))?.name || "Custom B";
  ...
  if (islA !== islB) {
    ...
    err.fromIsland = islA; err.toIsland = islB;
    throw err;
  }
